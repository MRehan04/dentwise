...
async function NextAppointment() {
    ...
    // filter upcoming CONFIRMED appointments only (today or future)
    const upcomingAppointments =
        ...
        if (!nextAppointment) return <NoNextAppointments />; // no upcoming appointments, return nothing

        const appointmentDate = parseISO(nextAppointment.date);
        const isToday = isSameDay(appointmentDate, new Date());
        const formattedDate = format(appointmentDate, "EEE, MMMM d, yyyy"); // e.g., Jan 1, 2024
